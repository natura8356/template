...
folder are mapped to the application's root namespace, 'npComponent'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "npComponent"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'npComponent' in the same way
as `"npComponent/sass/src"`.
